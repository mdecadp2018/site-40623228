...
if clientID!=-1:
  ...
  err,Bplayer_handle  =vrep.simxGetObjectHandle(clientID,'palyer1',vrep.simx_opmode_oneshot_wait)
  err,Bplayer_position=vrep.simxGetJointPosition(clientID,Bplayer_handle, vrep.simx_opmode_blocking)
  time.sleep(1)
  while (vrep.simxGetConnectionId(clientID) != -1):
    ...
    if err == vrep.simx_return_ok:
      ...
      # try to find something green
        ...
        #'''
        if ret_green != None:
            X = float(ret_green[1])-float(Bplayer_position[1])
            Y = float(ret_green[0])-float(Bplayer_position[0])
            if Bv<0.0:
